...
Among these, the most conventional measure widely accepted by scholars and policymakers is **nonfarm proprietors**. In addition, to this traditional measure, a research team at the Arizona State University suggests potentially meaningful alternative entrepreneurship measure in the digital era: **"venture density"**.

> "Percent of Non-farm Proprietors" measure is defined as share of nonfarm proprietors employment among total employments of a county. According to the Bureau of Economic Analysis, nonfarm proprietors employment **"consists of the number of nonfarm sole proprietorships and the number of individual general partners in nonfarm partnerships."**
...
> "Venture Density" measure is defined as the number of ventures per county divided by the population (i.e., ventures per 100 people). _"A venture (website) is a business, nonprofit, cause or an idea that a GoDaddy customer is working on, as represented by a discrete website and/or a domain name with services attached (email, payments, social media)" (Mossberger, Tolbert, & LaCombe, 2020, p.6)_

<div style="margin-bottom:30px;">
</div>

## Descriptive Statistics of Entrepreneurship Measures

```{r Impoart Libraries, results='hide', echo=FALSE, include=FALSE}
library(leaflet)
library(psych)
library(tidyverse)
...
tx_bb_entrepreneur_merged_v2 <- tx_bb_entrepreneur_merged_v2 %>% filter(ST == "TX")
...
desc_tab <- describe(tx_bb_entrepreneur_merged_v2[c("pct_50_est_cbp_2018","pct_10_est_cbp_2018","pct_nonfarmneest_nemp_2018","pct_nonfarm_bea_2018","venturedensity_mean")])

rownames(desc_tab) <- c("% of Business Establishment (<50 Employees)",
                               "% of Business Establishment (<10 Employees)",
                               "% of Nonfarm Nonemployer",
                               "% of Nonfarm Proprietors",
                               "Average Venture Density")
desc_tab <- desc_tab %>% select(n, mean, sd, median, min, max, range, skew, kurtosis) %>% 
  rename(N = n, Mean = mean, SD = sd, Median = median, Min = min, Max = max, Range = range, Skew = skew, Kurtosis = kurtosis)

knitr::kable(desc_tab, caption = "Table 1: Descriptive statistics of entrepreneurship measures",
             digits = 3, align = "rcccccccc") %>% kableExtra::kable_styling(full_width = F, position = "center")
...
```{r Contextual Table, echo=FALSE}
#### Contextual Table of Various Entrepreneurship Measures ####
cont_table <- tx_bb_entrepreneur_merged_v2 %>% 
  summarise(`Number of Nonfarm Nonemployer` = sum(nonfarmneest_nemp_2018, na.rm = T),
            `Number of Nonfarm Proprietors` = sum(nonfarmproprietors_bea_2018, na.rm = T),
            `Number of Establishments (<10 Employees)` = sum(est_10_cbp_2018, na.rm = T),
            `Total Business Establishments` = sum(est_cbp_2018, na.rm = T),
            `Total Employment` = sum(totalemp_bea_2018, na.rm = T))

cont_table <- as.data.frame(t(cont_table)) %>% rename(Total = V1)

knitr::kable(cont_table, caption = "Table 2: Business statistics of Texas (2018, Source: BEA, CBP)", format.args = list(big.mark = ","),
             align = "rc") %>% kableExtra::kable_styling(full_width = F, position = "center")
...
```{r Distributions of Entrepreneurship Measures, echo=FALSE, message=FALSE, fig.retina=2, fig.dim=c(10,7)}
grid.arrange(
  ...
  ggplot(tx_bb_entrepreneur_merged_v2, aes(x = venturedensity_mean)) + geom_histogram() +
    theme_minimal() + theme(axis.text = element_text(size = 11),
                            axis.title = element_text(size = 12, face = "bold")) +
    xlab("Venture Density (2018-19)") + ylab("Frequency") +
...
```{r Correlation Traditional Entre Measures, echo=FALSE, message=FALSE, fig.retina=2, fig.dim=c(15,5)}
grid.arrange(
...
```{r Correlation VD-Entre Measures, echo=FALSE, message=FALSE, fig.retina=2, fig.dim=c(15,5)}
grid.arrange(
...
```{r Nonfarm Nonemployer Share Map, echo=FALSE}
...
```{r Nonfarm Proprietor Share Map, echo=FALSE}
...
map %>% addTiles() %>% setView(lng = -100.000, lat = 31.000, zoom = 6) %>% 
  ...
  addLegend("bottomright", pal = pal2, values = ~venturedensity_mean,
            title = "Venture Density (2018-2019)", opacity = 1)
```
...
cbp_2012_2018 <- bind_rows(cbp2012_tx_p, cbp2013_tx_p, cbp2014_tx_p, cbp2015_tx_p, cbp2016_tx_p, cbp2017_tx_p, cbp2018_tx_p)
...
nonemp2012_tx_p <- nonemp2012_tx_raw %>% 
  ...
  left_join(., bea_county_2012_2018[c("county_FIPS","totalemp_2012")], by = "county_FIPS") %>% 
  rename(totalemp = totalemp_2012) %>% 
  mutate(pct_nonfarmneest_nemp = nonfarmneest_nemp / totalemp)

nonemp2013_tx_p <- nonemp2013_tx_raw %>% 
  mutate(est_total = ifelse(NAICS2012 == "00", NESTAB, 0),
         nonfarm_est_total = ifelse(str_detect(NAICS2012, "^11", negate = TRUE), NESTAB, 0),
         nonfarm_est_total = ifelse(NAICS2012 == "00", 0, nonfarm_est_total)) %>% 
  group_by(SECTOR, county_FIPS) %>% slice(1) %>% ungroup() %>% 
  ...
  summarise(neest_nemp = sum(est_total, na.rm = T),
            nonfarmneest_nemp = sum(nonfarm_est_total, na.rm = T),
            year = levels(factor(.$YEAR))) %>% 
  left_join(., bea_county_2012_2018[c("county_FIPS","totalemp_2013")], by = "county_FIPS") %>% 
  rename(totalemp = totalemp_2013) %>% 
  mutate(pct_nonfarmneest_nemp = nonfarmneest_nemp / totalemp)

nonemp2014_tx_p <- nonemp2014_tx_raw %>% 
  mutate(est_total = ifelse(NAICS2012 == "00", NESTAB, 0),
         nonfarm_est_total = ifelse(str_detect(NAICS2012, "^11", negate = TRUE), NESTAB, 0),
         nonfarm_est_total = ifelse(NAICS2012 == "00", 0, nonfarm_est_total)) %>% 
  group_by(SECTOR, county_FIPS) %>% slice(1) %>% ungroup() %>% 
  ...
  summarise(neest_nemp = sum(est_total, na.rm = T),
            nonfarmneest_nemp = sum(nonfarm_est_total, na.rm = T),
            year = levels(factor(.$YEAR))) %>% 
  left_join(., bea_county_2012_2018[c("county_FIPS","totalemp_2014")], by = "county_FIPS") %>% 
  rename(totalemp = totalemp_2014) %>% 
  mutate(pct_nonfarmneest_nemp = nonfarmneest_nemp / totalemp)

nonemp2015_tx_p <- nonemp2015_tx_raw %>% 
  mutate(est_total = ifelse(NAICS2012 == "00", NESTAB, 0),
         nonfarm_est_total = ifelse(str_detect(NAICS2012, "^11", negate = TRUE), NESTAB, 0),
         nonfarm_est_total = ifelse(NAICS2012 == "00", 0, nonfarm_est_total)) %>% 
  group_by(SECTOR, county_FIPS) %>% slice(1) %>% ungroup() %>% 
  ...
  summarise(neest_nemp = sum(est_total, na.rm = T),
            nonfarmneest_nemp = sum(nonfarm_est_total, na.rm = T),
            year = levels(factor(.$YEAR))) %>% 
  left_join(., bea_county_2012_2018[c("county_FIPS","totalemp_2015")], by = "county_FIPS") %>% 
  rename(totalemp = totalemp_2015) %>% 
  mutate(pct_nonfarmneest_nemp = nonfarmneest_nemp / totalemp)

nonemp2016_tx_p <- nonemp2016_tx_raw %>% 
  mutate(est_total = ifelse(NAICS2012 == "00", NESTAB, 0),
         nonfarm_est_total = ifelse(str_detect(NAICS2012, "^11", negate = TRUE), NESTAB, 0),
         nonfarm_est_total = ifelse(NAICS2012 == "00", 0, nonfarm_est_total)) %>% 
  group_by(SECTOR, county_FIPS) %>% slice(1) %>% ungroup() %>% 
  ...
  summarise(neest_nemp = sum(est_total, na.rm = T),
            nonfarmneest_nemp = sum(nonfarm_est_total, na.rm = T),
            year = levels(factor(.$YEAR))) %>% 
  left_join(., bea_county_2012_2018[c("county_FIPS","totalemp_2016")], by = "county_FIPS") %>% 
  rename(totalemp = totalemp_2016) %>% 
  mutate(pct_nonfarmneest_nemp = nonfarmneest_nemp / totalemp)

nonemp2017_tx_p <- nonemp2017_tx_raw %>% 
  ...
  summarise(neest_nemp = sum(est_total, na.rm = T),
            nonfarmneest_nemp = sum(nonfarm_est_total, na.rm = T),
            year = levels(factor(.$YEAR))) %>% 
  left_join(., bea_county_2012_2018[c("county_FIPS","totalemp_2017")], by = "county_FIPS") %>% 
  rename(totalemp = totalemp_2017) %>% 
  mutate(pct_nonfarmneest_nemp = nonfarmneest_nemp / totalemp)

nonemp2018_tx_p <- nonemp2018_tx_raw %>% 
  mutate(est_total = ifelse(NAICS2017 == "00", NESTAB, 0),
         nonfarm_est_total = ifelse(str_detect(NAICS2017, "^11", negate = TRUE), NESTAB, 0),
         nonfarm_est_total = ifelse(SECTOR == "00", 0, nonfarm_est_total)) %>% 
  group_by(SECTOR, county_FIPS) %>% slice(1) %>% ungroup() %>% 
  group_by(county_FIPS) %>% 
  summarise(neest_nemp = sum(est_total, na.rm = T),
            nonfarmneest_nemp = sum(nonfarm_est_total, na.rm = T),
            year = "2018") %>% 
  left_join(., bea_county_2012_2018[c("county_FIPS","totalemp_2018")], by = "county_FIPS") %>% 
  rename(totalemp = totalemp_2018) %>% 
  mutate(pct_nonfarmneest_nemp = nonfarmneest_nemp / totalemp)
...
nonemp_2012_2018 <- bind_rows(nonemp2012_tx_p, nonemp2013_tx_p, nonemp2014_tx_p, nonemp2015_tx_p, nonemp2016_tx_p,nonemp2017_tx_p, nonemp2018_tx_p)
...
nonemp_county_2012_2018 <- pivot_wider(data = nonemp_2012_2018,
                                    id_cols = county_FIPS,
                                    names_from = year,
                                    values_from = c("neest_nemp","nonfarmneest_nemp","pct_nonfarmneest_nemp","totalemp"))
# Summary table for the whole state of Texas
nonemp_tx_2012_2018 <- nonemp_2012_2018 %>% 
  group_by(year) %>% 
  summarise(totalemp = sum(totalemp),
            nonfarmnonemp_est = sum(nonfarmneest_nemp)) %>% 
  mutate(pct_nonfarm_nonemp = nonfarmnonemp_est / totalemp)
```
...
ggplot(cbp_2012_2018, aes(year, pct_50_est_cbp, group = county_FIPS, color = county_FIPS)) + 
  ...
  gghighlight(max(max(pct_50_est_cbp, na.rm = T) - min(pct_50_est_cbp, na.rm = T)),
              max_highlight = 7L,
              label_key = county,
              unhighlighted_params = list(size = 0.5, color = scales::alpha("grey", 0.3))) +
