...
* `pct_fixed_acs_2018`: Fixed broadband service subscription according to ACS 5-year estimates (%, 2018)
* `pct_bb_qos`: Compostive broadband quality of service (QoS) measure based on Microsoft's broadband through-put data (2019) and Mlab's broadband test data (2018-2019) (%)
...
* `pct_nonfarm_bea_2018`: Share of non-farm proprietors over total employment of a county (%, 2018; Source: BEA)
...
* `nonfarmprop_percapita`: Number of non-farm proprietors per capita (2018; Source: BEA)
...
* Log of `population_2018`: Natural log of county population (2018)
* `indstry_diversity`: Industry diversty index calculated based on methods described by Gallardo et al. (2021)
* `pctbachelors_2018`: % of people with Bachelor's degree (2018; Source: ACS 5-year estimates)
* `pct_unemployment_2018`: Unemployment rate (2018; Source: ACS 5-year estimates)
...
* `IRR2010`: Rurality index created by [Waldorf & Kim (2018)](https://purr.purdue.edu/publications/2960/1). Continuous measure ranging from 0 to 1
...
```{r subset data}
## Subset data with the aforementioned variables, create NLpop_2018 (natural log of population)
...
d2 <- d %>% select(county_FIPS, county.x,
                   ...
                   pct_fixed_acs_2018, pct_bb_qos,
                   pct_nonfarm_bea_2018, vd_mean_20, havd_mean_20, nonfarmprop_percapita,
                   population_2018, indstry_diversity, pctbachelors_2018, pct_unemployment_2018, RUCC_2013, metro_f, IRR2010) %>% 
  mutate(LNpop_2018 = log(population_2018))
...
d2 %>% select(pct25_3_dec_2019_fcc, pct100_10_dec_2019_fcc, pct250_25_dec_2019_fcc, pct1000_100_dec_2019_fcc,
             pct_fixed_acs_2018, pct_bb_qos) %>% summary()
...
d2 <- d2 %>% 
  mutate(BB_25_3_high = case_when(pct25_3_dec_2019_fcc < mean(pct25_3_dec_2019_fcc, na.rm = T) ~ 0,
         ...
         BB_1000_100_high = case_when(pct1000_100_dec_2019_fcc < mean(pct1000_100_dec_2019_fcc, na.rm = T) ~ 0,
                                  ...
                                  TRUE ~ NA_real_),
         BB_adoption_high = case_when(pct_fixed_acs_2018 < mean(pct_fixed_acs_2018, na.rm = T) ~ 0,
                                  pct_fixed_acs_2018 > mean(pct_fixed_acs_2018, na.rm = T) ~ 1,
                                  TRUE ~ NA_real_),
...
popcut <- hist(d2$LNpop_2018, br=seq(min(d2$LNpop_2018, na.rm = T), max(d2$LNpop_2018, na.rm = T), length.out = 4), plot = F)$breaks
indstrycut <- hist(d2$indstry_diversity, br=seq(min(d2$indstry_diversity), max(d2$indstry_diversity), length.out = 4), plot = F)$breaks
educut <- hist(d2$pctbachelors_2018, br=seq(min(d2$pctbachelors_2018), max(d2$pctbachelors_2018), length.out = 4), plot = F)$breaks
unempcut <- hist(d2$pct_unemployment_2018, br=seq(min(d2$pct_unemployment_2018), max(d2$pct_unemployment_2018), length.out = 4), plot = F)$breaks

mycp <- list(LNpop_2018 = popcut, indstry_diversity = indstrycut, pctbachelors_2018 = educut, pct_unemployment_2018 = unempcut)

## List of covariates
vars <- c("LNpop_2018", "indstry_diversity", "pctbachelors_2018", "pct_unemployment_2018")
droplst <- c("pct_nonfarm_bea_2018", "vd_mean_20", "havd_mean_20", "nonfarmprop_percapita", "RUCC_2013", "metro_f", "IRR2010")

#### Treatment: BB_25_3_high ####

d25_3 <- d2 %>% select(BB_25_3_high,
                    pct_nonfarm_bea_2018, vd_mean_20, havd_mean_20, nonfarmprop_percapita,
                    LNpop_2018, indstry_diversity, pctbachelors_2018, pct_unemployment_2018,
                    RUCC_2013, metro_f, IRR2010)
...
#### Treatment: BB_25_3_high & Outcome: pct_nonfarm_bea_2018 ####

mod_25_3_nonfarm <- att(match_fcc_25_3, pct_nonfarm_bea_2018 ~ BB_25_3_high * metro_f, data = d25_3)
summary(mod_25_3_nonfarm)

#### Treatment: BB_25_3_high & Outcome: vd_mean_20 ####

mod_25_3_vd <- att(match_fcc_25_3, vd_mean_20 ~ BB_25_3_high * metro_f, data = d25_3)
summary(mod_25_3_vd)

#### Treatment: BB_25_3_high & Outcome: havd_mean_20 ####

mod_25_3_havd <- att(match_fcc_25_3, havd_mean_20 ~ BB_25_3_high * metro_f, data = d25_3)
summary(mod_25_3_havd)

#### Treatment: BB_25_3_high & Outcome: nonfarmprop_percapita ####

mod_25_3_nonfarmpc <- att(match_fcc_25_3, nonfarmprop_percapita ~ BB_25_3_high * metro_f, data = d25_3)
...
#### Treatment: BB_100_10_high & Outcome: pct_nonfarm_bea_2018 ####

mod_100_10_nonfarm <- att(match_fcc_100_10, pct_nonfarm_bea_2018 ~ BB_100_10_high * metro_f, data = d100_10)
summary(mod_100_10_nonfarm)

#### Treatment: BB_100_10_high & Outcome: vd_mean_20 ####

mod_100_10_vd <- att(match_fcc_100_10, vd_mean_20 ~ BB_100_10_high * metro_f, data = d100_10)
summary(mod_100_10_vd)

#### Treatment: BB_100_10_high & Outcome: havd_mean_20 ####

mod_100_10_havd <- att(match_fcc_100_10, havd_mean_20 ~ BB_100_10_high * metro_f, data = d100_10)
summary(mod_100_10_havd)

#### Treatment: BB_100_10_high & Outcome: nonfarmprop_percapita ####

mod_100_10_nonfarmpc <- att(match_fcc_100_10, nonfarmprop_percapita ~ BB_100_10_high * metro_f, data = d100_10)
...
#### Treatment: BB_250_25_high & Outcome: pct_nonfarm_bea_2018 ####

mod_250_25_nonfarm <- att(match_fcc_250_25, pct_nonfarm_bea_2018 ~ BB_250_25_high * metro_f, data = d250_25)
summary(mod_250_25_nonfarm)

#### Treatment: BB_250_25_high & Outcome: vd_mean_20 ####

mod_250_25_vd <- att(match_fcc_250_25, vd_mean_20 ~ BB_250_25_high * metro_f, data = d250_25)
summary(mod_250_25_vd)

#### Treatment: BB_100_10_high & Outcome: havd_mean_20 ####

mod_250_25_havd <- att(match_fcc_250_25, havd_mean_20 ~ BB_250_25_high * metro_f, data = d250_25)
summary(mod_250_25_havd)

#### Treatment: BB_100_10_high & Outcome: nonfarmprop_percapita ####

mod_250_25_nonfarmpc <- att(match_fcc_250_25, nonfarmprop_percapita ~ BB_250_25_high * metro_f, data = d250_25)
...
#### Treatment: BB_1000_100_high & Outcome: pct_nonfarm_bea_2018 ####

mod_1000_100_nonfarm <- att(match_fcc_1000_100, pct_nonfarm_bea_2018 ~ BB_1000_100_high * metro_f, data = d1000_100)
summary(mod_1000_100_nonfarm)

#### Treatment: BB_100_10_high & Outcome: vd_mean_20 ####

mod_1000_100_vd <- att(match_fcc_1000_100, vd_mean_20 ~ BB_1000_100_high * metro_f, data = d1000_100)
summary(mod_1000_100_vd)

#### Treatment: BB_100_10_high & Outcome: havd_mean_20 ####

mod_1000_100_havd <- att(match_fcc_1000_100, havd_mean_20 ~ BB_1000_100_high * metro_f, data = d1000_100)
summary(mod_1000_100_havd)

#### Treatment: BB_100_10_high & Outcome: nonfarmprop_percapita ####

mod_1000_100_nonfarmpc <- att(match_fcc_1000_100, nonfarmprop_percapita ~ BB_1000_100_high * metro_f, data = d1000_100)
...
mod_adoption_nonfarm <- att(match_fcc_adoption, pct_nonfarm_bea_2018 ~ BB_adoption_high * metro_f, data = dadoption)
summary(mod_adoption_nonfarm)

#### Treatment: BB_100_10_high & Outcome: vd_mean_20 ####

mod_adoption_vd <- att(match_fcc_adoption, vd_mean_20 ~ BB_adoption_high * metro_f, data = dadoption)
summary(mod_adoption_vd)

#### Treatment: BB_100_10_high & Outcome: havd_mean_20 ####

mod_adoption_havd <- att(match_fcc_adoption, havd_mean_20 ~ BB_adoption_high * metro_f, data = dadoption)
summary(mod_adoption_havd)

#### Treatment: BB_100_10_high & Outcome: nonfarmprop_percapita ####

mod_adoption_nonfarmpc <- att(match_fcc_adoption, nonfarmprop_percapita ~ BB_adoption_high * metro_f, data = dadoption)
...
#### Treatment: BB_qos_high & Outcome: pct_nonfarm_bea_2018 ####

mod_qos_nonfarm <- att(match_fcc_qos, pct_nonfarm_bea_2018 ~ BB_qos_high * metro_f, data = dqos)
summary(mod_qos_nonfarm)

#### Treatment: BB_qos_high & Outcome: vd_mean_20 ####

mod_qos_vd <- att(match_fcc_qos, vd_mean_20 ~ BB_qos_high * metro_f, data = dqos)
summary(mod_qos_vd)

#### Treatment: BB_qos_high & Outcome: havd_mean_20 ####

mod_qos_havd <- att(match_fcc_qos, havd_mean_20 ~ BB_qos_high * metro_f, data = dqos)
summary(mod_qos_havd)

#### Treatment: BB_qos_high & Outcome: nonfarmprop_percapita ####

mod_qos_nonfarmpc <- att(match_fcc_qos, nonfarmprop_percapita ~ BB_qos_high * metro_f, data = dqos)
