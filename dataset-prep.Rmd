...
ms_broadband_tx <- ms_broadband_tx %>% mutate(county = `COUNTY NAME`)
str(ms_broadband_tx)
...
tx_bb_entrepreneur_merged <- tx_bb_entrepreneur_merged %>% 
  ...
  select(-c("FIPS2010","state.x","state.y","ST",`COUNTY ID`,`COUNTY NAME`,
...
tx_bb_entrepreneur_merged_v2 <- tx_bb_entrepreneur_merged %>%  
  ...
  left_join(., select(rucc2013, c("FIPS","RUCC_2013","metro_desc")), by = "FIPS")
