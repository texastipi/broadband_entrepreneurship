...
View(listCensusMetadata(name = "acs/acs5", vintage = 2019, type = "variables"))
...
median_income <- getCensus(name = "acs/acs5",
                           vintage = 2019,
                           vars = "B06011_001E",
                           region = "county:*", # Total median household income
                           regionin = "state:20,23,48")
...
median_income <- median_income %>% 
  rename(med_income_2019 = B06011_001E) %>% 
  mutate(med_income_2019 = med_income_2019/1000,
         county_FIPS = paste0(state, county)) %>% 
  select(-c(state, county))

# Merge
demo_cont <- left_join(demo_cont, median_income, by = "county_FIPS")
...
## Calculate Distances to the Nearest Metro Area

In this section, I will calculate the distances from each county to the nearest metro area. The definition of metro area follows the Census Bureau's Core-based Statistical Area (Metro/micropolitan areas). In this dataset, I include only the CBSA areas with a total population of 100,000 or greater.

Here, I use the `tidycensus` package and `tigris`package for data retrieval.

Set up your Census API key for the `tidycensus` package. For detailed tutorial and introduction to using `tidycensus` and `tigris`, Kyle Walker provides awesome sets of tutorials such as [this](https://walker-data.com/census-r/an-introduction-to-tidycensus.html).

```{r tidycensus setup, eval=FALSE, results='hide'}
library(tidycensus)
...
library(tidycensus)
census_api_key("9001b546c2d77876a089119664dc25a4235eea37", install = TRUE, overwrite = TRUE)
```

```{r calculate distances, results='hide'}
library(sf)
library(tigris)
#### Use Census Tigris and Census Data ####
## Workflow:
## 1) Retrieve all CBSA (metro) area shapefile
## 2) Retrieve all county shapefile
## 3) Generate centroids for CBSA polygons
## 4) Generate centroids for county shapefile
## 5) For each county centroids, calculate distance to all CBSA centroids and select the minimum value
##    (i.e., distance to closest CBSA metro/micropolitan area)

## Useful resources: Kyle Walkers blog. (Specifically: https://walker-data.com/census-r/spatial-analysis-with-us-census-data.html)

## Retrieve CBSA population with geometries
cbsa_pop <- get_acs(
  geography = "cbsa",
...
# Save simple feature with point geometry of centroids of CBSA
cbsa_cent <- cbsa_pop %>% st_transform(crs = 4326) %>% st_centroid()    # CRS specified

## Retrieve county with geometries
county_shp <- counties()
# Save simple feature with point geometry of centroids of counties
county_cent <- county_shp %>% st_transform(crs = 4326) %>% st_centroid()
# Select useful variables
county_cbsa_dist <- county_cent %>% 
  select(STATEFP, COUNTYFP, GEOID, ALAND, geometry)

## Loop through all CBSAs and retrieve minimum distance to each county centroid
min_dist_df <- data.frame()   # Creating a blank data frame
# Loop through each county observation, calculate distances to all CBSA, get the minimum distance, row bind to the blank data frame
for (row in 1:nrow(county_cbsa_dist)) {
  min_dist <- as.vector(min(st_distance(county_cbsa_dist[row,], cbsa_cent)))
  min_dist_df <- rbind(min_dist_df, min_dist)
}
# Set the column name
colnames(min_dist_df) <- c("cbsa_min_dist")
glimpse(min_dist_df)

# Merge to the county centroid data frame
county_cbsa_dist <- county_cbsa_dist %>% 
  bind_cols(min_dist_df) %>% 
  st_drop_geometry()
glimpse(county_cbsa_dist)
```

## Add Social Capital Index

Here I will add a variable of interest. An index of social capital is provided by Penn State University [here](https://aese.psu.edu/nercrd/community/social-capital-resources). Downloaded data is stored on the GitHub page.

```{r social capital index, results='hide'}
## Import dataset
sc_indx <- read_csv("https://raw.githubusercontent.com/texastipi/broadband_entrepreneurship/master/social_capital_index_2014.csv")
...
tx_bb_entrepreneur_merged_v2 <- tx_bb_entrepreneur_merged %>%  
  ...
  left_join(., select(county_cbsa_dist, GEOID, ALAND, cbsa_min_dist), by = c("county_FIPS" = "GEOID")) %>% 
  left_join(., sc_indx, by = c("county_FIPS" = "FIPS"))
