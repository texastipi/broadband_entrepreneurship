...
bea_nonfarmproprietor_txksme <- getBEAstat(c(2012, 2017, 2018), beaKey = beaKey) %>% mutate(pct_chg_bea_2012_2018 = pct_nonfarm_bea_2018 - pct_nonfarm_bea_2012)
...
tx_bb_entrepreneur_merged_v2 <- tx_bb_entrepreneur_merged_v2 %>% 
  ...
  mutate(pct_broadband_MS = case_when(is.na(pct_broadband_MS) ~ 0, TRUE ~ pct_broadband_MS),
...
tx_bb_entrepreneur_merged_v2 <- tx_bb_entrepreneur_merged_v2 %>% 
  mutate(pct_bb_qos = normalize((scale(pct_broadband_MS)[,] + scale(pct_broadband_mlab)[,]),
                                      range = c(0,100))/100)
